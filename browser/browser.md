...
- 线程共享内存, 但一崩全崩, 难怪以前用 firefox 遇见过一崩全崩 😆

牺牲了多进程切换的性能, windows 下用多进程因为没有 fork 每次都要慢慢扫描压缩数据, 没有写时复制, 所以 chrome 吃内存

> [来源](https://www.zhihu.com/question/368712837)

[标签页可能会合并, 一个网站的就会合并](https://www.zhihu.com/question/22829613)

可以在 chrome 的任务管理器看, 一个进程的话前面灰线会标志出来
...
显示器每秒钟刷新一个固定的频率, 也就是每秒更新这么多张图片, 显卡运算后将图片放在缓冲区, 显示器从中取出显示

浏览器渲染引擎也就是渲染一帧图像然后放入显卡缓冲区, [步骤如下](https://juejin.im/entry/590801780ce46300617c89b8)

- 重排(回流), 计算布局
- 重绘, 计算颜色
- 合成

将渲染树转换的层树, 然后每层转换为绘画指令列表, 再光栅化, 然后将这些图层合成为一站图片发往缓冲区, 合成操作在合成线程中运行, 所以主线程阻塞, css 动画仍能绘制

> [来源](https://segmentfault.com/a/1190000014520786)
...
> [来源](http://blog.poetries.top/browser-working-principle/guide/part5/lesson24.html)
